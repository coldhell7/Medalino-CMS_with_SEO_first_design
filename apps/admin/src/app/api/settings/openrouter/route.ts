...
import {
  ...
  writeOpenRouterApiKeyToFile,
} from "@/lib/site-settings";

export const runtime = "nodejs";

export async function GET() {
  const envSet = Boolean(process.env.OPENROUTER_API_KEY?.trim());
  const fileKey = readOpenRouterApiKeyFromFile();
...
export async function POST(req: Request) {
  ...
  writeOpenRouterApiKeyToFile(apiKey);
  return NextResponse.json({
    ok: true,
    source: process.env.OPENROUTER_API_KEY?.trim() ? "env" : "file",
    maskedKey: maskApiKey(getEffectiveOpenRouterApiKey() ?? ""),
  });
}
